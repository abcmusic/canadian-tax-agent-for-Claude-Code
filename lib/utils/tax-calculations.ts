...
export interface CCPCTaxResult {
  ...
  sbdIncome: number;  // Income eligible for SBD
  sbdTax: number;  // Tax on SBD income
  generalRateIncome: number;  // Income over SBD limit
  generalRateTax: number;
  federalTax: number;  // Total federal (SBD + general)
  provincialTax: number;
  investmentTax: number;  // Tax on investment income
  rdtoh: number;  // Refundable Dividend Tax On Hand
  smallBusinessDeduction: number;  // Deprecated: use sbdTax
  totalTax: number;
  effectiveRate: number;
}
...
/**
 * CPP (Canada Pension Plan) limits and rates for 2025
 * Source: CRA
 * NOTE: CPP is federal and applies to all provinces
 */
export const CPP_LIMITS_2025 = {
  maxPensionableEarnings: 68500,
  basicExemption: 3500,
  employeeRate: 0.0595,  // 5.95%
  selfEmployedRate: 0.119,  // 11.9% (2x employee rate)
  maxContributionEmployee: 3867.50,  // (68500 - 3500) * 0.0595
  maxContributionSelfEmployed: 7735.00  // (68500 - 3500) * 0.119
};

/**
 * EI (Employment Insurance) limits and rates for 2025
 * Source: Service Canada
 * NOTE: EI is federal and applies to all provinces (except Quebec has separate QPIP)
 */
export const EI_LIMITS_2025 = {
  maxInsurableEarnings: 63200,
  employeeRate: 0.0166,  // 1.66%
  employerRate: 0.02324,  // 2.324% (1.4x employee rate)
  maxPremiumEmployee: 1049.12,  // 63200 * 0.0166
  maxPremiumEmployer: 1468.77   // 63200 * 0.02324
};
...
export function calculateCCPCTax(
  ...
  // Federal tax total
  const federalTax = sbdTax + generalRateTax;
  ...
  const rdtoh = investmentIncome * 0.3067; // 30.67% of investment income
  ...
  return {
    ...
    investmentTax,
    rdtoh,
    smallBusinessDeduction: sbdTax, // Deprecated alias
    totalTax,
